...
const Summary = ({ activeStep, onNextStep, onPreviousStep }) => {
  ...
  const [charCount, setCharCount] = useState(0);
  const navigate = useNavigate();
  const { formDataContext, updateSummary } = useFormContext(); // Use the context

  // Set the initial content of the text area when the component mounts
  useEffect(() => {
    if (textAreaRef.current) {
      textAreaRef.current.innerText = formDataContext.summary;
      setCharCount(formDataContext.summary.length);
  ...
  const handleInput = (e) => {
    ...
    setCharCount(e.target.innerText.trim().length);
    updateSummary(e.target.innerText.trim()); // Update the context with the new summary
  };
  ...
  const handleSubmit = () => {
    navigate("/preview", { state: formDataContext }); // Pass form data to the Preview component
  };
