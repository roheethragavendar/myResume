...
// const Education = ({ activeStep, onNextStep, onPreviousStep }) => {
//     const navigate = useNavigate();
//     const { formDataContext, updateEducation } = useFormContext();

//     const [education, setEducation] = useState(formDataContext.education);

//     useEffect(() => {
//         setEducation(formDataContext.education);
//       }, [formDataContext.education]);
...
//     const handleInputChange = (e, id) => {
//         const { name, value } = e.target;
//         const updatedEducation = workHistory.map((entry) =>
//             entry.id === id ? { ...entry, [name]: value } : entry
//           );
...
//             endYear: "",
//         };
//         const updatedEducation = [...workHistory, newEntry];
//         setEducation(updatedEducation);
//         updateEducation(updatedEducation);
//     };
...
//     const handleDelete = (id) => {
//         const updatedEducation = (formData.filter((data) => data.id !== id));
//         setEducation(updatedEducation);
//         updateEducation(updatedEducation);
//     };
...
const Education = ({ activeStep, onNextStep, onPreviousStep }) => {
    ...
    const validateForm = () => {
        ...
        education.forEach((data) => {
            ...
            if (data.Degree === "Enter a different degree" && !data.differentDegree.trim()) {
                newErrors[`differentDegree-${data.id}`] = "Degree is required";
            }
            if (!data.fieldOfStudy.trim()) {
                newErrors[`fieldOfStudy-${data.id}`] = "Field Of Study is required";
            }
    ...
    const handleDegreeChange = (selectedOption, id) => {
        const updatedEducation = education.map((entry) =>
            entry.id === id ? { ...entry, Degree: selectedOption?.value, differentDegree: selectedOption?.value === "Enter a different degree" ? "" : entry.differentDegree } : entry
        );
        setEducation(updatedEducation);
        updateEducation(updatedEducation);
        setSelectedDegree(selectedOption?.value);
    ...
    useEffect(() => {
        if (formDataContext.education.length === 0) {
            ...
            updateEducation(defaultEducation);
        } else {
            setEducation(formDataContext.education);
        }
    }, [formDataContext.education]);
    ...
    const handleDelete = (id) => {
        const updatedEducation = education.filter((data) => data.id !== id);
        setEducation(updatedEducation);
        updateEducation(updatedEducation);
    };
                                ...
                                <div className="flex flex-wrap gap-4 sm:gap-6 gap-y-6 w-full sm:w-full">
                                    ...
                                    <div className="flex flex-col text-[#002D6B] font-semibold text-sm sm:text-[15px] uppercase w-full sm:max-w-[300px] lg:max-w-[500px]">
                                        ...
                                        <Select
                                            ...
                                            placeholder="Select"
                                            onChange={(selectedOption) => handleDegreeChange(selectedOption, entry.id)}
                                            value={optionsThree.find(option => option.value === entry.Degree)}
                                        />
                                        {errors[`Degree-${entry.id}`] && (
                                            <p className="text-red-500 text-sm">{errors[`Degree-${entry.id}`]}</p>
                                        )}
                                    ...
                                    {entry.Degree === "Enter a different degree" && (
                                        <div className="flex flex-col text-[#002D6B] font-semibold text-sm sm:text-[15px] uppercase w-full sm:max-w-[300px] lg:max-w-[500px]">
                                            <label htmlFor={`differentDegree-${entry.id}`} className="mb-2">
                                                Enter a different degree *
                                            </label>
                                            <input
                                                type="text"
                                                name="differentDegree"
                                                id={`differentDegree-${entry.id}`}
                                                placeholder="e.g. Bachelor's"
                                                className="w-full text-[18px] font-normal border border-[#002D6B] h-14 px-3 focus:outline-none focus:border-[#026A73] focus:border-2"
                                                value={entry.differentDegree}
                                                onChange={(e) => handleInputChange(e, entry.id)}
                                            />
                                            {errors[`differentDegree-${entry.id}`] && (
                                                <p className="text-red-500 text-sm">{errors[`differentDegree-${entry.id}`]}</p>
                                            )}
                                        </div>
                                    )}
                                </div>
                                    ...
                                    <div className="flex flex-col sm:flex-row gap-4 sm:gap-6">
                                        <div className="flex flex-col text-[#002D6B] font-semibold text-sm sm:text-[15px] capitalize w-full sm:max-w-[240px]">
                                            <Select
                                                ...
                                                value={options.find(option => option.value === entry.endMonth)} // Ensure the correct value is displayed
                                            />
                                        </div>
                                        ...
                                        <div className="flex flex-col text-[#002D6B] font-semibold text-sm sm:text-[15px] capitalize w-full sm:max-w-[240px]">
                                            <Select
                                                ...
                                                value={optionsTwo.find(option => option.value === entry.endYear)} // Ensure the correct value is displayed
                                            />
                                        </div>
                                    </div>
                                </div>
